...
// ✅ FIXED: Connected to Public Blog API
// ============================================
...
const Home = () => {
  ...
  useEffect(() => {
    const fetchBlogs = async () => {
      try {
        // Use the new public route we just added to app.js
  ...
  const getImageUrl = (filename) => {
    ...
    // Get base URL (e.g., https://backend.onrender.com)
    ...
    try {
      ...
      if (response.data.success) {
        ...
        if (role === 'admin' || role === 'super-admin') {
          navigate('/admin'); // Or /admin-panel based on your route
        } else {
          navigate('/dashboard');
        }
      ...
      <div style={{ background: 'linear-gradient(135deg, #1e40af 0%, #7c3aed 100%)', padding: '5rem 3rem' }}>
        <div style={{ maxWidth: '1400px', margin: '0 auto', textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: '1.25rem', justifyContent: 'center' }}>
            <Link to="/register" style={{ padding: '1.125rem 2.75rem', background: 'white', color: '#1e40af', textDecoration: 'none', borderRadius: '0.75rem', fontWeight: 700, fontSize: '1.125rem' }}>
              Get Started Free
            </Link>
            <Link to="/login" style={{ padding: '1.125rem 2.75rem', background: 'rgba(255, 255, 255, 0.15)', color: 'white', textDecoration: 'none', borderRadius: '0.75rem', fontWeight: 700, fontSize: '1.125rem', border: '2px solid white' }}>
              Sign In
            </Link>
          </div>
        </div>
                ...
                <div key={blog.blog_id || blog._id} style={{ 
                  ...
                  <div style={{ padding: '2rem', flex: 1, display: 'flex', flexDirection: 'column' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.85rem', color: '#64748b', marginBottom: '1rem' }}>
                      <Calendar size={14} />
                      {/* Handle both snake_case and camelCase dates */}
      ...
      {showAdminLogin && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999 }} onClick={() => setShowAdminLogin(false)}>
          <div style={{ background: 'white', padding: '2.5rem', borderRadius: '1.25rem', width: '450px', position: 'relative' }} onClick={e => e.stopPropagation()}>
            ...
            {errors.submit && <div style={{ background: '#fee2e2', color: '#dc2626', padding: '0.75rem', borderRadius: '0.5rem', marginBottom: '1rem' }}>{errors.submit}</div>}
            ...
            <form onSubmit={handleAdminLogin}>
              <div style={{ marginBottom: '1rem' }}>
                ...
                <div style={{ position: 'relative' }}>
                  ...
                  <input type="email" value={adminForm.email} onChange={e => setAdminForm({...adminForm, email: e.target.value})} style={{ width: '100%', padding: '0.75rem 0.75rem 0.75rem 2.5rem', border: '2px solid #e5e7eb', borderRadius: '0.5rem' }} required placeholder="admin@example.com" />
                </div>
              </div>
