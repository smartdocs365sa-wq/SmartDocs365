// ============================================
// FILE: Backend/controllers/registerController.js (ENHANCED)
// AUTO FREE TRIAL ASSIGNMENT ON REGISTRATION
// ============================================
...
const suncriptionTypesModel = require("../models/subcriptionTypesModel.js");
...
const create = async (req, res, next) => {
  ...
  //Validate entered email already exists or not
  try {
  ...
  try {
    // ✅ Create user with FREE TRIAL plan
    let result = await userModel.create({
      ...
      user_id,
      mobile,
      plan_id: FREE_TRIAL_PLAN_ID, // Auto-assign Free Trial
      created_at,
      updated_at,
    });

    // ✅ Create FREE TRIAL recharge entry (30 days, 10 PDFs)
    var recharge_expiry_date = addDaysToCurrentDate(30);
    ...
    var rechargePayload = {
      ...
      is_active: true,
      payment_status: true, // Auto-approved for free trial
    ...
    if (result) {
      ...
      res.status(201).json({
        success: true,
        message: `User Registration Successful! Free Trial (30 days, 10 PDFs) activated.`,
      });
    } else {
...
const getUserDetail = async (req, res, next) => {
  try {
    ...
    for (var i = 0; i < rechargeInfos.length; i++) {
      ...
      var planInfo = await suncriptionTypesModel.findOne({ plan_id });
...
const getUserSubcription = async (req, res, next) => {
  try {
    ...
    for (var i = 0; i < rechargeInfos.length; i++) {
      var recharge_id = rechargeInfos[i]?.recharge_id;
      var plan_id = rechargeInfos[i]?.plan_id;
      planInfo = await suncriptionTypesModel.findOne({ plan_id });
      totalLimit = totalLimit + planInfo?.pdf_limit;
      var pdfDataForThisRechargeId = await pdfDetailsModel.find({ recharge_id });
      pdfCount = pdfCount + pdfDataForThisRechargeId.length;
      isRechargeActive = true;
